---
title: Pacioli Manual
---

# Pacioli Manual

Manual for Pacioli {{ site.pacioli_version }}

---

## Contents

- [Programs](program/contents)
  - [Program structure](program/structure)
  - [Namespaces](program/namespaces)
  - [Imports and includes](program/imports)
- [Definitions and Declarations](definitions/contents)
  - [Value Definition](definitions/define)
  - [Type Declaration](definitions/declare)
  - [Index Set Definition](definitions/defindex)
  - [Base Unit Definition](definitions/defunit)
  - [Unit Alias Definition](definitions/defalias)
  - [Type Definition](definitions/deftype)
  - [Matrix Definition](definitions/defmatrix)
  - [Conversion Definition](definitions/defconv)
  - [Record Definition](definitions/defrecord)
- [Units of Measurement](uom/contents)
  - [Scalar Units](uom/scalar-units)
  - [Units in Types](uom/units-in-types)
  - [Units at Runtime](uom/units-at-runtime)
  - [Predefined Units](uom/predefined-units)
- [Types](types/contents)
  - [The Type System](types/type-system)
  - [Type Variables](types/type-variables)
  - [Matrix Type](types/matrix-type)
  - [Index Type](types/index-type)
  - [Tuple Type](types/tuple-type)
  - [Function Type](types/function-type)
  - [User Defined Types](types/generics)
  - [Predefined Types](types/predefined-types)
- [Expressions](expressions/contents)
  - [Unit notation](expressions/unit-notation)
  - [Functions and Applications](expressions/functions)
  - [Operators](expressions/operators)
  - [Tuples](expressions/tuples)
  - [Lists and Comprehensions](expressions/lists-and-comprehensions)
  - [Conditional (if then else)](expressions/conditional)
  - [Binding (let in)](expressions/binding)
  - [Imperative Blocks (begin end)](expressions/statements)
- [Web Components](web-components/contents)
  - [Introduction](web-components/introduction)
  - [pacioli-value](web-components/pacioli-value)
  - [pacioli-matrix](web-components/pacioli-matrix)
  - [pacioli-table](web-components/pacioli-table)
  - [pacioli-bar-chart](web-components/pacioli-bar-chart)
  - [pacioli-pie-chart](web-components/pacioli-pie-chart)
  - [pacioli-histogram](web-components/pacioli-histogram)
  - [pacioli-line-chart](web-components/pacioli-line-chart)
  - [pacioli-scatter-plot](web-components/pacioli-scatter-plot)
  - [pacioli-scene](web-components/pacioli-scene)
  - [pacioli-inputs](web-components/pacioli-inputs)
  - [pacioli-controls](web-components/pacioli-controls)
