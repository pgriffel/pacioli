...
public class DesugarStatements extends IdentityTransformation implements Visitor {
	...
	// Issue: place reference can end up in closures. Create a lambda/application pair
	...
	public void visit(StatementNode node) {
        ...
        // Create a catch block around the statement body that uses the result place for the result
	...
	public void visit(IfStatementNode node) {
		returnNode(new BranchNode(expAccept(node.test), expAccept(node.positive), expAccept(node.negative), node.getLocation()));
	}

	@Override
	public void visit(ReturnNode node) {
		returnNode(newCall(node.getLocation(), "Primitives", "throw_result", statementResult.peek(), expAccept(node.value)));
	}
	...
	public void visit(WhileNode node) {
		ExpressionNode testCode = new LambdaNode(new ArrayList<String>(), expAccept(node.test), node.test.getLocation());
        ExpressionNode bodyCode = new LambdaNode(new ArrayList<String>(), expAccept(node.body), node.body.getLocation());
