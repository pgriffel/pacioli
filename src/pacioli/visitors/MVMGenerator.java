...
public class MVMGenerator implements Visitor {
	...
	public void visit(IndexSetDefinition node) {
        ...
        out.format("indexset \"%s\" \"%s\" list(%s);\n",
        		node.globalName(), node.localName(), Utils.intercalate(",", quotedItems));
	}
	...
	public void visit(UnitDefinition node) {
		...
		if (number == null) {
            ...
            out.format("unit \"%s\" \"%s\" %s %s;\n", node.getName(), node.getSymbol(),
            		number.factor(), Utils.compileUnitToMVM(number.unit()));
        }
	}
	...
	public void visit(ApplicationNode node) {
        if (settings.debug() && node.function instanceof IdentifierNode) {
            ...
            out.format("application_debug(\"%s\", \"%s\", \"%s\", ",
            		escapeString(stackText), escapeString(fullText), traceOn);
            id.accept(this);
...
//            return String.format("application(%s%s)", node.function.compileToMVM(settings), args);
        }
	...
	public void visit(MatrixLiteralNode node) {
		...
		*/
		...
		// Find the matrix type's row and column dimension. They should have been set during resolving 
		/*MatrixDimension rowDim = compileTimeMatrixDimension(matrixType.rowDimension);
		MatrixDimension columnDim = compileTimeMatrixDimension(matrixType.columnDimension);*/		
		...
		// Write the elements. Table check stores all found indices to check for doublures. 
